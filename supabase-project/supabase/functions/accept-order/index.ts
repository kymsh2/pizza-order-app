...
serve(async (req) => {
  ...
  try {
    ...
    const { data: order, error: fetchError } = await supabase
      .from("orders")
      .select(
        "id, status, wc_order_id, total, payment_method, created_at, customers(name, email), order_items(name, quantity, total, size)",
      )
      .eq("id", order_id)
      .single();
    ...
    if (customer?.email) {
      const emailResponse = await fetch(
        `${Deno.env.get("SUPABASE_URL")}/functions/v1/send-order-confirmation-email`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!}`,
            "x-api-key": Deno.env.get("KITCHEN_APP_API_KEY")!,
          },
          body: JSON.stringify({
            customer_email: customer.email,
            customer_name: customer.name,
            order_number: updatedOrder.wc_order_id,
            pickup_time: updatedOrder.pickup_at,
      ...
      console.warn("Order has no customer email", { order_id });
    }

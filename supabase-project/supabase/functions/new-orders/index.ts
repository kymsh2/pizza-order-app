...
serve(async (req) => {
  ...
  for (const order of orders) {
    ...
    if (selectError) {
      console.error("Order select error:", selectError);
      throw selectError;
    }
    ...
    if (existingOrder) {
      console.log(
        `Skipping WC order ${order.id}, already exists with status ${existingOrder.status}`
      );
      continue;
    }
...
function mapWcStatusToAppStatus(wcStatus: string): string {
  switch (wcStatus) {
    case 'pending':
    case 'checkout-draft':
    case 'on-hold':
